...
class TopTen(scrapy.Spider):
    ...
    def start_requests(self):
        return [scrapy.FormRequest("http://bbs.byr.cn/user/ajax_login.json",
                                   ...
                                   callback=self.get_topten)]

    def get_topten(self, response):
        topten_url = 'https://bbs.byr.cn/rss/topten'
        top_req = requests.get(topten_url)
        top_req.encoding = 'gb2312'
        pattern = re.compile('<link>(.*)</link>')
        start_urls = pattern.findall(top_req.text)
        print(start_urls)
        if len(start_urls) != 11:
            return
